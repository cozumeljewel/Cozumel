-- ============================================================
-- MIGRACIÓN v12 · el SKU deja de vivir solo dentro del email
--
-- Pega esto en el SQL Editor de Supabase (proyecto "ADRI",
-- ddcrkglgdbasbxanbjkc) y pulsa Run. No toca tablas, datos ni políticas:
-- solo añade una función y una vista, y reescribe el disparador del email
-- para que use la función en vez de llevar su propia copia de la lógica.
--
-- Motivo: hasta ahora el SKU de EMANCO se calculaba DENTRO del email de
-- "pedido pagado". Al exportar los pedidos a Excel para pasárselos al
-- proveedor, en la tabla "reservas" no había ninguna columna con la
-- referencia: tocaba abrir los emails uno a uno o cruzarlo a mano.
--
-- Deja dos cosas:
--   1) public.sku_de_pedido(producto, personalizacion) → la referencia
--      exacta a pedir (acabado + mes + grabado ya aplicados). Es la misma
--      lógica de v10, movida a una función para que el email y la
--      exportación no puedan acabar diciendo cosas distintas.
--   2) public.pedidos_proveedor → vista lista para exportar a CSV desde
--      Supabase (Table Editor → pedidos_proveedor → Export to CSV), con
--      una fila por pieza: pedido, fecha, SKU, grabado y datos de envío.
...
begin
  -- Pedidos anteriores al selector de acabado no traen esta clave: oro.
  acabado := lower(coalesce(personalizacion->>'acabado', 'oro'));

  -- Acabado por pieza de los kits: cada una puede ir en un acabado.
  ...
  return case producto
    ...
    -- El collar de dos cadenas se pide con el SKU combinado, no con sus
    -- dos sub-SKU; el grabado va como línea aparte: "diaoke".
    ...
    -- El sufijo -KZ ya incluye el grabado: no se añade diaoke.
...
-- ---- 2) La vista que se exporta a Excel ----
-- Una fila por pieza. Las piezas de un mismo pedido comparten "pedido"
-- (el stripe_session_id) y los mismos datos de envío.
...
select
  ...
  nullif(trim(concat_ws(' · ',
    nullif(r.personalizacion->>'nombre', ''),
    nullif(r.personalizacion->>'fecha', ''),
    nullif(r.personalizacion->>'mensaje', ''),
    nullif(r.personalizacion->>'grabado', '')
  )), '')                                             as grabado,
...
comment on view public.pedidos_proveedor is
  'Pedidos pagados listos para exportar a CSV y pedir a EMANCO. Una fila por pieza; sku_a_pedir ya viene resuelto con acabado, mes y grabado.';

-- La vista consulta "reservas", que tiene RLS: desde el panel de Supabase
-- (service_role) se ve todo, y desde el navegador cada persona solo vería
-- lo suyo. No abre nada nuevo.
grant select on public.pedidos_proveedor to authenticated;

-- ---- 3) El email de pedido pagado, ahora usando la función ----
...
begin
  ...
  -- El SKU lo resuelve ahora public.sku_de_pedido (arriba), que es la
  -- MISMA funcion que usa la vista de exportacion: una sola copia de la
  -- logica, asi el email y el Excel no pueden decir cosas distintas.
  sku_pedido := public.sku_de_pedido(new.producto, new.personalizacion);
