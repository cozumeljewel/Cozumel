-- ============================================================
-- MIGRACIÓN v9 · el email de pedido calcula el SKU exacto a pedir
--
-- Pega esto en el SQL Editor de Supabase (proyecto "ADRI",
-- ddcrkglgdbasbxanbjkc) y pulsa Run.
--
-- Motivo: el email interno decía "Collar Destino" y "Acabado: oro", pero
-- nunca la referencia de EMANCO que hay que pedir — eso solo vivía en
-- productos.js (código de la web), así que alguien tenía que cruzarlo a
-- mano cada vez. Ahora el email trae ya la línea "SKU a pedir" resuelta:
-- pieza + acabado + mes (si aplica) + sufijo de grabado, todo junto.
--
-- Solo toca el email al NEGOCIO. El email al cliente no cambia: a él no
-- le importa la referencia del proveedor.
--
-- ⚠️ El orden de los meses (enero=W1 ... diciembre=W12) es una suposición
-- por el orden del Excel de SKU, NUNCA confirmada con EMANCO. Si el
-- proveedor confirma un orden distinto, hay que corregirlo aquí abajo,
-- en el bloque "mes_num :=".
--
-- Si mañana cambia un SKU o se añade una pieza nueva, hay que actualizar
-- ESTE archivo (créate una v10) Y productos.js — no se comparten entre sí,
-- igual que ya pasa con los precios en precios.ts. Ver ese archivo para
-- el mismo aviso.
-- ============================================================
...
begin
  ...
  -- mes, acabado...) en una sola línea legible, en el mismo orden en que
  -- vengan. Si no hay ninguna, se queda en null y ese bloque no sale.
  grabado_texto := null;
  ...
  sku_pedido := case new.producto
    ...
    when 'kit_pedacito_nosotros' then
      -- Collar Esencia + Pulsera Dos Almas
      (case when acabado = 'plata' then 'CDNN067-1 + YS14924A0W0' else 'CDNN067-2 + YS14924D0W0' end) ||
      (case when tiene_grabado then ' + diaoke' else '' end)

    when 'kit_mi_consentida' then
      -- Collar Destino + Pulsera Mi Cielo (el -KZ de la pulsera ya lleva grabado)
      case when mes_num is null then
        '⚠️ Falta el mes en el pedido — revisar a mano'
      else
        (case when acabado = 'plata'
          then 'XX49472A0W' || mes_num::text || ' + YS15777A0W0-KZ'
          else 'XX49472D0W' || mes_num::text || ' + YS15777D0W0-KZ'
        end)
      end
  ...
  -- ---- Email al cliente (sin cambios respecto a v8) ----
  ...
  -- ---- Email al negocio: ahora con el SKU resuelto, en negrita ----
  perform net.http_post(
    url := 'https://api.resend.com/emails',
    headers := jsonb_build_object(
      'Authorization', 'Bearer ' || resend_key,
    ...
    body := jsonb_build_object(
      ...
      '<tr><td style="color:#4E9A9B;">Acabado</td><td>' || initcap(acabado) || '</td></tr>' ||
